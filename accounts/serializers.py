...
class RegisterDepartmentSerializer(ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        user = Department.objects.create(
            email=validated_data["email"], dept_name=validated_data["dept_name"]
        )
...
class DepartmentSerializer(ModelSerializer):
    ...
    president_signature = ImageField(
        write_only=True, allow_empty_file=True, required=False
    )
    secretary_signature = ImageField(
        write_only=True, allow_empty_file=True, required=False
    )
